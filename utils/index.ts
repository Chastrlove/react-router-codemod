...
export function submoduleImport(j, root, moduleName, submoduleName) {
  return root
    .find(j.ImportDeclaration, {
      source: {
        value: moduleName,
...
export function hasSubmoduleImport(j, root, moduleName, submoduleName) {
  return submoduleImport(j, root, moduleName, submoduleName).length > 0;
}

export function hasModuleImport(j, root, moduleName) {
  return (
...
export function hasModuleDefaultImport(j, root, pkgName, callback) {
  let found = false;
  root
    .find(j.ImportDeclaration, {
    ...
    .forEach((nodePath) => {
      ...
      if (defaultImport.length) {
        found = true;
        callback(defaultImport.at(0));
      }
    });
  return found;
}

export function hasModuleNamespaceImport(j, root, pkgName, callback) {
  let found = false;
  root
    .find(j.ImportDeclaration, {
      source: { value: pkgName },
...
function hasModuleDefaultLocalImport(j, root, pkgName, localModuleName) {
  let found = false;
  root
    .find(j.ImportDeclaration, {
      source: { value: pkgName },
